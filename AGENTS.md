...
This devenv is meant to be imported by a game decomp. Data paths are `DEVENV_ROOT` (the importing project): disc dump in `game/`, matching C in `src/`, Psy-Q in `tools/psyq/`, `report.json` at the repo root.
